...
def milestone_view(request, id_disp):
    ...
    doc = Document.objects.filter(content_type = content_type, \
    objects_id = mil_obj.pk )
...
def overdueview(request):
    todays = datetime.date.today()
    overdue_list = Milestone.objects.filter(due_date__lt=todays,status=False)
    project_list=Project.objects.all()
    clients=Client.objects.all()
    #status = Milestone.objects.filter(status = False)

    #dispp = request.GET.get('id')
    #miel = Milestone.objects.all()
    #taks = Tasks.objects.all()
    
    return render(request, "milestones/milestone_overdue.html", locals())
...
def addmilestone(request, task=None, id=None):
    ...
    #todays = datetime.date.today()+timedelta(days=7)
        ...
        if request.POST:
            ...
            available_task = request.POST.get('available_task', '')
            if form.is_valid():
                f = form.save(commit=False)
...
def mile(request):
    ...
    if status_id == '2':
        milestone_list = milestone_list.filter(status=status_id)
        #milestone_list = milestone_list.exclude(status='1')
        #milestone_list=milestone_list.exclude(status='0')
    if milestone_id:
...
def milestone_owned_by(request):
    ...
    milestone__list = Milestone.objects.filter(owned_by=user_list)

    return render(request, "milestones/ownedbyme.html", locals())
