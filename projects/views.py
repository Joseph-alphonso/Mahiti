    ...
    if task == 'add':
            ...
            if form.is_valid():
                ...
                if not check:
                    ...
                    if project_manager:
                        ...
                        project_obj.save()
                    userprofile_obj = UserProfile.objects.get(email=request.user.email)
                    if userprofile_obj.access_level == '1':
                            userprofile_obj.project.add(project_obj)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    project_obj.project_manager = Project_Manager.objects.get(id=int(project_manager))
                except:
                    project_obj.project_manager = None
                else:
                    email = project_obj.project_manager.userprofile.email
                    url = 'http://pms.mahiti.org/project/view/'+str(project_obj.id)+'/'
...
def project_dashboard(request, id_disp):
    ...
    if key == 'unclosed':
        tasks_unclosed_list = a.get_unclosed_task_list()
        return render(request, "projects/unclosed-tasks.html", locals())

    if key == 'no_assignee':
        tasks_list = a.get_unclosed_task_list()
        print tasks_list
        tasks_no_assignee_list = a.get_no_assignee_task_list()
        return render(request, "projects/tasks-with-no-assignee.html", locals())
