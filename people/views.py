...
def person_list(request):
    ...
    if request.method == "POST":
        ...
        if access_level:
            peoples_list = peoples_list.filter(access_level=access_level)
        client_id = request.POST.get('client')
        if client_id:
            peoples_list = peoples_list.filter(client=client_id)
        first_name = request.POST.get('person-name', '')
        if first_name:
            peoples_list = peoples_list.filter(Q(first_name__icontains=first_name)| 
...
#                check = UserProfile.objects.filter(first_name=first_name)
                ...
                userprofile = UserProfile.objects.create(user=usr, first_name=first_name, 
                    ...
                    phone=phone, network=network, website=website, notes=notes, 
                    profile_image=profile_image, access_level=access_level, 
...
def useredit(request, task=None, id=None):
    ...
    if task == "edit":
        ...
        form = UserEditForm(initial={'profile_image':userobj.profile_image, 
                ...
                'network': userobj.network, 'access_level': userobj.access_level, 
                'website': userobj.website, 'notes': userobj.notes, 
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                project_managerobj = UserProfile.objects.filter(access_level="2")
                if project_managerobj:
                    project_manager_obj = Project_Manager.objects.get_or_create(userprofile=project_managerobj)
                edit_done = True
                success = True
